--[[
Atlasloot Enhanced
Author Daviesh / Reworked by Deim of PrimalWoW / Adjusted by Gescht
...
ATLASLOOT_VERSION = "|cffFF8400Atlas Loot|r";
	...
	if (dataID == "SearchResult") or (dataID == "WishList") or (AtlasLoot_IsLootTableAvailable(dataID)) then
		...
		for i = 1, 30, 1 do
			...
			if(dataSource[dataID][i] ~= nil and dataSource[dataID][i][4] ~= "") then
				...
				--[[
				Custom addition for Bluering private server
				blueRingPhase will contain information about what phase was called
				blueRingHeroic will be true if the heroic mode is toggled on, the default will be mythic
				]]
				--is there a dataID
				if dataID then
...
function AtlasLoot_GetLODModule(dataSource)
	...
	elseif (dataSource=="AtlasLootBlueRing") then
		return "AtlasLoot_BlueRing";
	end
end
...
function AtlasLoot_LoadAllModules()
	local orig, bc, wotlk, craft, world, bluering;
    ...
    bluering, _ = LoadAddOn("AtlasLoot_BlueRing");
    local flag=0;
	if not orig then
    ...
    if not bluering then
		LoadAddOn("AtlasLoot_BlueRing");
		flag=1;
	end
	if flag == 1 then
		if ATLASLOOT_DEBUGMESSAGES then
